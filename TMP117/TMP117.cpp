...
{
	Wire.beginTransmission(_deviceAddress); // Originally cast (uint8_t)
	Wire.write(reg);
	int ret = Wire.endTransmission();					   // endTransmission but keep the connection active
	if(ret == 0){
          printf("!!!WARN: TMP117 I2C Error\n");
          SetErrorMask(TEMP_ERROR_MASK);
        }
        ret = Wire.requestFrom(_deviceAddress, (uint8_t)2); // Ask for 2 bytes, once done, bus is released by default
        if(ret == 0){
          printf("!!!WARN: TMP117 I2C Error\n");
          SetErrorMask(TEMP_ERROR_MASK);
        }
...
{
	Wire.beginTransmission(_deviceAddress); // Originally cast uint8_t when a register value again
	Wire.write(reg);
	Wire.write(data>>8); // Write MSB (D15-D8)
	Wire.write(data);  // Write LSB (D7-D0)
	int ret = Wire.endTransmission();	 // Stop transmitting data
        if(ret == 0){
          printf("!!!WARN: TMP117 I2C Error\n");
          SetErrorMask(TEMP_ERROR_MASK);
        }
