...
void Dps310::begin(TwoWire &bus, uint8_t slaveAddress)
{
	...
	//nrf_delay_ms(50);		//startup time of Dps310
...
int16_t Dps310::measureTempOnce(int32_t &result, uint8_t oversamplingRate)
{
	...
	nrf_delay_ms(calcBusyTime(0U, m_tempOsr)/DPS310__BUSYTIME_SCALING);
	nrf_delay_ms(DPS310__BUSYTIME_FAILSAFE);
...
int16_t Dps310::measurePressureOnce(int32_t &result, uint8_t oversamplingRate)
{
	...
	//wait until measurement is finished
	nrf_delay_ms(calcBusyTime(0U, m_prsOsr)/DPS310__BUSYTIME_SCALING);
	nrf_delay_ms(DPS310__BUSYTIME_FAILSAFE);
...
int16_t Dps310::readByte(uint8_t regAddress)
{
	...
	{
                 printf("!!!WARN: DPS310 I2C Error\n");
                 SetErrorMask(PRESS_ERROR_MASK);
