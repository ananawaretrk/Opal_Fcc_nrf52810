...
const struct hci_cmd_desc_tag hci_cmd_desc_tab_dialog_vs[] =
{
    ...
    CMD(ADC_VBAT                 , DBG, 0, PK_GEN_GEN, NULL                   , "H"                            ),
#if defined (__DA14531__)
...
#if defined (__DA14531__)
    event->vbat_lvl = (uint16_t) adc_531_get_vbat_sample();
#else
...
#if defined (__DA14531__) && defined (CFG_HW_RESET_P00)
        // Disable hw_reset functionality.
        if ((gpio_port == GPIO_PORT_0) && (gpio_pin == GPIO_PIN_0))
        {
...
#if defined (__DA14531__) && defined (CFG_HW_RESET_P00)
    // Restore hw_reset functionality.
    set_hw_reset();
#endif
...
#if !defined (__DA14531__)
    if ((port == GPIO_PORT_1 && pin == GPIO_PIN_4) ||
        (port == GPIO_PORT_1 && pin == GPIO_PIN_5))
#else
...
#endif
    {
        // Disable JTAG for using the pins as GPIOs.
        while (GetBits16(SYS_STAT_REG, DBG_IS_UP) != 0);
        SetBits16(SYS_CTRL_REG, DEBUGGER_ENABLE, 0);
    }
