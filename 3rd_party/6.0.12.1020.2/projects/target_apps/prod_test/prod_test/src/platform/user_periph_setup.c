...
/****************************************************************************************/
uint8_t port_sel                            __SECTION_ZERO("prodtest_uninit");
...
void update_uart_pads(GPIO_PORT tx_port, GPIO_PIN tx_pin, GPIO_PORT rx_port, GPIO_PIN rx_pin)
{
    ...
    if ( ( tx_pin == GPIO_PIN_2 ) || 
         ( rx_pin == GPIO_PIN_2 ) ||
         ( debugger_enable == 1 && ( (tx_pin == GPIO_PIN_5 ) || ( rx_pin == GPIO_PIN_5 ) ) ) ||
         ( debugger_enable == 2 && ( (tx_pin == GPIO_PIN_1 ) || ( rx_pin == GPIO_PIN_1 ) ) ) ||
         ( debugger_enable == 3 && ( (tx_pin == GPIO_PIN_10 ) || ( rx_pin == GPIO_PIN_10 ) ) ) )
#else
    ...
    if (((uart_sel_pins.uart_port_tx == GPIO_PORT_0) && (uart_sel_pins.uart_tx_pin == GPIO_PIN_0)) ||
        ((uart_sel_pins.uart_port_rx == GPIO_PORT_0) && (uart_sel_pins.uart_rx_pin == GPIO_PIN_0)))
    {
        GPIO_Disable_HW_Reset();
    }
    else
