...
struct hci_acl_data_tx
{
    ...
    uint16_t    length;
    #if (BLE_EMB_PRESENT)
    /// Pointer to the first descriptor containing TX Data
    struct co_buf_tx_node *desc;
#if defined (__DA14531__) && !defined (__ES2_DA14531__)
    /// Pointer to the data buffer
    uint8_t* buffer;
    /// How many bytes have been processed so far by LLC
    uint16_t index;
    // Number of fragments that this packet was fragmented by LLC
    uint8_t desc_cnt;
#endif
    #else // (BLE_HOST_PRESENT)
    /// Pointer to the data buffer
...
#if BT_EMB_PRESENT
// TODO [VLE] variable size: structure definition + special pack/unpack + handler
...
// TODO [LT] find solution to make it generic or hide LE commands from BT single mode
