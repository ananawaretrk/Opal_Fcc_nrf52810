...
extern const struct prf_task_cbs *prf_itf_get(uint16_t task_id);
...
void prf_init_func(bool reset)
{
    ...
    {
        // FW boot profile initialization
        while (i < BLE_NB_USED_PROFILES)
        {
            prf->env  = NULL;
            prf->task = TASK_RFU_5 + i +1;
            prf->id   = TASK_ID_INVALID;

            // Initialize Task Descriptor
            prf->desc.default_handler = NULL;
            prf->desc.state           = NULL;
            prf->desc.idx_max         = 0;

            prf->desc.state_max       = 0;
            prf->desc.state_handler   = NULL;

            ke_task_create(prf->task, &(prf->desc));
    ...
    else
    {
        ...
        while (i < BLE_NB_USED_PROFILES)
        {
            ...
            // Request kernel to flush task messages
            ke_task_msg_flush(KE_TYPE_GET(prf->task));
