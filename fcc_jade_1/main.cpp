...
{ 
  ...
  if(num_rand_bytes_available > 0){
    sd_rand_application_vector_get(rand_number, 3);
  }
  ...
  if (rand_number[2] == byte_old_radom_number)
  {
      int err = sd_rand_application_bytes_available_get(&num_rand_bytes_available);
      if (num_rand_bytes_available > 0)
      {
          sd_rand_application_vector_get(&rand_number[2], 2);
      }
  }
  ...
  byte_old_radom_number = rand_number[2];
...
sm_state start_gatt_server()
{
    ...
    //STATE_BLE_CONT_CW_TX
    if(MODE == 2 && lora_selected == 0 && advTime == 0 && scanDuration == 0 && sleepTime == 0){
    return STATE_BLE_CONT_CW_TX;
    }

    else if(MODE == 0 && lora_selected == 0 && advTime == 0 && scanDuration == 0 && sleepTime == 0){
    return STATE_SLEEP;
    }
    
    //STATE_BLE_CONT_MCW_TX
    else if(MODE == 3 && lora_selected == 0 && advTime == 0 && scanDuration == 0 && sleepTime == 0){
    return STATE_BLE_CONT_MCW_TX;
    }

    //STATE_BLE_CONT_RX
    else if(MODE == 2 && lora_selected == 0 && advTime == 0 && scanDuration > 0 && sleepTime == 0){
...
sm_state lora_cont_rx()
{
    // Init LoRa
    printf(DBG_GREEN "LoRa Radio Continous Transmit\n" DBG_RESET);
    loraInit();
    nrf_delay_ms(1000);
...
sm_state lora_int_rx()
{
    // Init LoRa
    printf(DBG_GREEN "LoRa Radio Interval Receive\n" DBG_RESET);
    loraInit();
    nrf_delay_ms(1000);
...
sm_state lora_int_cw_tx()
{
    ...
    while (1)
    {
        printf(DBG_GREEN "LoRa Radio CW Transmit\n" DBG_RESET);
        lora_continuous_cw_transmit();
        ...
        printf(DBG_GREEN "LoRa Radio stop\n" DBG_RESET);
        lora_disable();
...
sm_state lora_fixed_frequency_hopping()
{
    ...
    lora_frequency_set[0] = 917.1;
    for(int i=0; i<32; i++)
    {
      lora_frequency_set[i+1] = lora_frequency_set[i] + 0.2;
    }
    for(int i=0; i<32; i++)
    {
...
sm_state debug_function()
{
    ...
    printf(DBG_MAGENTA "LoRa Radio stop\n" DBG_RESET);
...
int main(void)
{
    ...
    while (1)
    {
        switch (state)
        {
            ...
            case STATE_LORA_CONT_CW_TX:
                printf("STATE_LORA_CONT_CW_TX\n");
                state = lora_cont_cw_tx();
                break;

            case STATE_LORA_CONT_MCW_TX:
                printf("STATE_LORA_CONT_MCW_TX\n");
                state = lora_cont_mcw_tx();
                break;

            case STATE_LORA_CONT_RX:
                printf("STATE_LORA_CONT_RX\n");
                state = lora_cont_rx();
                break;

            case STATE_LORA_INT_RX:
                printf("STATE_LORA_INT_RX\n");
                state = lora_int_rx();
                break;

            case STATE_LORA_INT_CW_TX:
                printf("STATE_LORA_INT_CW_TX\n");
                state = lora_int_cw_tx();
                break;

            case STATE_LORA_INT_MCW_TX:
                printf("STATE_LORA_INT_MCW_TX\n");
                state = lora_int_mcw_tx();
                break;
