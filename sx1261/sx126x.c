...
int8_t SX126xGetRssiInst( void )
{
    ...
    SX126xReadCommand( RADIO_GET_RSSIINST, buf, 2 );
    rssi = -1 * (buf[1] >> 1);
    return rssi;
}
...
void SX126xGetPacketStatus( PacketStatus_t *pktStatus )
{
    ...
    SX126xReadCommand( RADIO_GET_PACKETSTATUS, status, 4 );

    pktStatus->packetType = SX126xGetPacketType( );
    switch( pktStatus->packetType )
    {
        case PACKET_TYPE_GFSK:
            pktStatus->Params.Gfsk.RxStatus = status[1];
            pktStatus->Params.Gfsk.RssiSync = -status[2] >> 1;
            pktStatus->Params.Gfsk.RssiAvg = -status[3] >> 1;
            pktStatus->Params.Gfsk.FreqError = 0;
            break;

        case PACKET_TYPE_LORA:
            pktStatus->Params.LoRa.RssiPkt = -status[1] >> 1;
            ( status[2] < 128 ) ? ( pktStatus->Params.LoRa.SnrPkt = status[2] >> 2 ) : ( pktStatus->Params.LoRa.SnrPkt = ( ( status[2] - 256 ) >> 2 ) );
            pktStatus->Params.LoRa.SignalRssiPkt = -status[3] >> 1;
