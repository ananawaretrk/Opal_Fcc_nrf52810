...
#define FREQ_STEP                                   ( double )( XTAL_FREQ / (double) FREQ_DIV )
...
public:
    ...
    void        spiReadAddr(uint16_t addr, uint8_t* dest, uint8_t len);
    uint8_t     spiReadReg(uint16_t reg);
    void        spiReadBuffer(uint8_t offset, uint8_t *buffer, uint8_t size);
    uint32_t    spiWriteCommand(RadioCommands_t cmd, const uint8_t* src, uint8_t len);
    void        spiWriteAddr(uint16_t addr, const uint8_t* src, uint8_t len);
    void        spiWriteBuffer(uint8_t offset, uint8_t *buffer, uint8_t size);

    bool        available(void);
    bool        checkPacketSent();
    ...
    int         frequencyError(void);
    int16_t     lastRssi(void);
...
private:
    ...
    PacketParams_t        _Pparams;
    ModulationParams_t    _Mparams;

    uint8_t               _rxPayload[MAX_PAYLOAD_LEN];
    uint8_t               _rxPayloadSize;
    bool                  _rxBufValid;
    uint32_t              _rxGood;
    PacketStatus_t        _packetStatus;

    bool                  _rxContinuous;

    uint8_t               _header[HEADER_LEN];
