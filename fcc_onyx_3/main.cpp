...
#define CELL_ENABLE_PIN_O        _PINNUM(1, 6)
#define CELL_TX                  26
#define CELL_RX                  27
...
int8_t cellpowerlevel = 0;
long cell_input_min = 0;
long cell_input_max = 8;
long cell_output_min = -50;
long cell_output_max = 23;

int8_t cellrxpowerlevel = 0;
long cellrx_input_min = 0;
long cellrx_input_max = 8;
long cellrx_output_min = -127;
long cellrx_output_max = -25;

void cell_interval_receive(int rcv_time);
void cell_interval_receive_modulated(int rcv_time);
...
APP_TIMER_DEF(cell_timer_id); // use only for cellular operations
...
void nus_data_handler(ble_nus_evt_t * p_evt)
{
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        celltxrx = p_evt->params.rx_data.p_data[5];
...
void tapeDiagnosis(void)
{
    ...
    // CELL
      ...
      printf(DBG_GREEN "Cell Radio Selected\n" DBG_RESET);
    }
    ...
    if(debugMode) // TRUE -> interval
    {
        while(1)
        {   
            // Cell - enabled and transmit
            if(celltxrx == 1 && advTime > 0)
            {
                printf(DBG_GREEN "Cell CW Radio Transmit\n" DBG_RESET);
                nbiot_instance.CwFunction(1, 1, channel, cellpowerlevel, false);
            }
            
            // BLE - enabled and transmit
            else if(celltxrx == 0 && advTime > 0)
            {
                printf(DBG_BLUE "BLE Radio Transmit\n" DBG_RESET);
                radio_config();
            }
            
            // Cell/BLE Advertise time
            if(advTime > 0)
            {
            start_timer(advTime);
            ...
            {
                #ifdef test_print
                printf(DBG_GREEN "Cell Radio Disabled\n" DBG_RESET);
                #endif //test_print
            ...
            if (sleepTime != 0)
            {
                printf(DBG_YELLOW "Cell/BLE Radio Sleep\n" DBG_RESET);
            ...
            // Cell - enabled and receive
            if ((celltxrx == 2) && scanDuration >0)
            {
                printf(DBG_GREEN "CELL Interval Radio receive\n" DBG_RESET);
                printf(DBG_MAGENTA "-------scanDuration: %d\n" DBG_RESET, scanDuration);
                cell_interval_receive(scanDuration);
            }

              
            // BLE - enabled and receive
            else if (celltxrx == 0 && scanDuration >0)
            {
...
//                stop_timer();
    ...
    else
    {
        ...
        else if(celltxrx == 2 && scanDuration == 0 && advTime == 0)
        {
    ...
    if(flag) // TRUE -> interval
    {
        ...
        {   
            ...
            if (celltxrx == 2 && scanDuration >0)
            {
                printf(DBG_GREEN "Cell Interval Radio receive\n" DBG_RESET);
                printf(DBG_MAGENTA "-------scanDuration: %d\n" DBG_RESET, scanDuration);
                cell_interval_receive_modulated(scanDuration);
            }
...
//                stop_timer();
    ...
    else
    {
        if (celltxrx == 1)
        {
            printf(DBG_GREEN "Cell Modulated Radio Transmit\n" DBG_RESET);
            nbiot_instance.CwFunction(1, 1, channel, cellpowerlevel, true);
        }
        else if (celltxrx == 2)
        {
            printf(DBG_GREEN "Cell Modulated Radio Receive\n" DBG_RESET);
            nbiot_instance.CwFunction(0, 1, channel, cellpowerlevel, true);
        }
        else if (celltxrx == 0)
        {
            printf(DBG_BLUE "BLE Radio Transmit\n" DBG_RESET);
...
void setConfig(int setTime)
{
    ...
    if((channel < 0 || channel > 100) && celltxrx == 0)
    {
        channel = 17;
    }
    ...
    cellpowerlevel = (int8_t)power_map(txlevel, cell_input_min, cell_input_max, cell_output_min, cell_output_max);
    cellrxpowerlevel = (int8_t)power_map(txlevel, cellrx_input_min, cellrx_input_max, cellrx_output_min, cellrx_output_max);
    ...
    printf(DBG_MAGENTA "Cell Radio TX or RX: %d\n" DBG_RESET, celltxrx);
    printf(DBG_MAGENTA "channel: %d\n" DBG_RESET, channel);
    printf(DBG_MAGENTA "cellpowerlevel: %d\n" DBG_RESET, cellpowerlevel);
    printf(DBG_MAGENTA "cellrxpowerlevel: %d\n" DBG_RESET, cellrxpowerlevel);
...
void CellTimerEventHandler(void *p_context) {
  cell_timer_flag = 1;
}

void StopCellTimer() {
  app_timer_stop(cell_timer_id);
}

void StartCellTimer(uint32_t mili_sec_timeout) {
  //printf("----------------- Starting Cell Timer ------------------\n");
  uint32_t error_code = 0;
  StopCellTimer();
  app_timer_create(&cell_timer_id, APP_TIMER_MODE_SINGLE_SHOT, CellTimerEventHandler);
  error_code = app_timer_start(cell_timer_id, APP_TIMER_TICKS(mili_sec_timeout), NULL);
  cell_timer_flag = 0;
}

void init_Modem()
{
  printf(DBG_BKG_CYAN "Initializing modem\n" DBG_RESET);
...
void cell_interval_receive(int rcv_time)
{
    ...
    printf(DBG_MAGENTA "cell Interval Receive\n" DBG_RESET);
    ...
    nbiot_instance.CwFunction(0, 0, channel, cellrxpowerlevel, false);
}
